...
class SimpleLinearModel(object):
    ...
    def __cal_r_square(self):
        tmp_sum_y_hat = 0
        tmp_sum_y = 0
        for i in range(len(self.__x_data)):
            tmp_sum_y_hat += pow((self.__b0 + self.__b1 * self.__x_data[i] - self.__mean_y), 2)
            tmp_sum_y += pow((self.__y_data[i] - self.__mean_y), 2)
        self.__r_square = tmp_sum_y_hat / tmp_sum_y
